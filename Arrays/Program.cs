// See https://aka.ms/new-console-template for more information
using Arrays;

Console.WriteLine("Hello, World!");

// ArrayUtils.plusMinus(new List<int>() { 1, 1, 0, -1, -1 });

// ArrayUtils.miniMaxSum(new List<int> { 1, 2, 3, 4, 5 });
// ArrayUtils.miniMaxSum(new List<int> { 7, 69, 2, 221, 8974 });
// ArrayUtils.miniMaxSum(new List<int> { 1, 3, 5, 7, 9 });
// ArrayUtils.miniMaxSum(new List<int> { 1, 1, 1, 1, 1 });

// Console.WriteLine(ArrayUtils.findMedian(new List<int> { 3, 0, 6, 4, 1, 2, 5 }));
// Console.WriteLine(ArrayUtils.findMedian(new List<int> { 3, 0, 6 }));

//Console.WriteLine(ArrayUtils.lonelyinteger([1, 2, 3, 4, 3, 2, 1]));

ArrayUtils.flippingMatrix(new List<List<int>>());